...
export default function EditProfessorScreen() {
  ...
  const selectItem = (p) => {
    setSelected(p);
    setForm({ full_name: p.full_name || '', department: p.department || '', university: p.university || '' });
  };
  ...
  const save = async () => {
    try {
      ...
      await loadList(''); // refresco inmediato (sin esperar al debounce)
        ...
        <>
          ...
          {loading ? <ActivityIndicator style={{ marginTop: 12 }} /> :
            list.length === 0 ? <Text style={{ color: '#6b7280', padding: 16 }}>No se han encontrado resultados.</Text> :
              <FlatList data={list} keyExtractor={(it) => String(it.id)} renderItem={({ item }) => <Item item={item} onPress={selectItem} />} />
          }
        </>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          <Text style={{ color: '#111827', fontSize: 18, fontWeight: '800', marginBottom: 8 }}>Editar Profesor</Text>
          <Row label="Nombre completo" value={form.full_name} onChangeText={(v) => setForm({ ...form, full_name: v })} />
          <Row label="Departamento" value={form.department} onChangeText={(v) => setForm({ ...form, department: v })} />
          <Row label="Universidad" value={form.university} onChangeText={(v) => setForm({ ...form, university: v })} />
