...
export default function DeleteProfessorScreen() {
  const { user } = useAuth();
  ...
  const confirmDelete = (prof) => {
    Alert.alert(
      'Eliminar Profesor',
      `Se borrará el profesor y TODOS sus vínculos y reseñas.\n\n${prof.full_name}`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setLoading(true);
              const res = await ProfessorService.deleteProfessorCascade(prof.id);
              if (!res.success) throw new Error(res.error);

              // ✅ Optimista: quita el item ya
              setList(prev => prev.filter(p => p.id !== prof.id));

              // ✅ Mensaje de éxito
              Alert.alert('Listo', 'Profesor eliminado correctamente.');

              // ✅ Refresco inmediato usando el filtro actual (sin esperar debounce)
              await loadList(q);
            } catch (e) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      <SearchBarDebounced value={q} onChangeText={setQ} placeholder="Buscar profesor…" />
